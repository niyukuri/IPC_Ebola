...
recent_first_fosa_evaluations <- function(data_hz = data_hz){
  ...
  for(i in 1:length(table(data_hz_df$nom_fosa_new))){
    ...
    if(j>1){
      ...
      scores_first_i <- dt_f$score_pci_100[(nrow(dt_f) - (nrow(dt_f)-1))]
      ...
      if(is.na(dt_f$nbre_partenaires[nrow(dt_f)-1])){
        partenaires_first_i <- 0
      }else{
        partenaires_first_i <- dt_f$nbre_partenaires[nrow(dt_f)-1]
      }
  ...
  hz_ev <- data.frame(nom_zone_sante_hz, fosa, categorie_fosa, scores_recent, scores_first, partenaires_first, partenaires_recent)
  return(hz_ev)
