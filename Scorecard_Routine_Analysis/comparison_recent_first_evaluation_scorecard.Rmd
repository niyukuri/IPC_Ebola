---
title: "Analysis of difference between first and recent IPC evaluations"
...
for(i in 1:length(table(data_hz_df$nom_fosa_new))){
  ...
  if(j>1){
    ...
    first_evaluation <- dt_f[,c(1, 156:167, 169)][1,]
    
    names(first_evaluation) <- paste0("1st_", names(first_evaluation))
    
    
    recent_evaluation <- dt_f[,c(1, 156:167, 169)][nrow(dt_f),]
    
    names(recent_evaluation) <- paste0("last_", names(recent_evaluation))
    
    first_last_df_i <- cbind(zamaho, first_evaluation, recent_evaluation)
    
    first_last_df <- rbind(first_last_df, first_last_df_i)
...
data_nziza_descriptive_recent <- first_last_df
...
# Comparing of IPC score for the first and recent evaluations
...
first_eval <- compa_dat_1[,13]
recent_eval <- compa_dat_2[,13]

df_evals <- data.frame(first_eval, recent_eval)

score <- c(df_evals$first_eval, df_evals$recent_eval)

eval <- c(rep("First", length(df_evals$first_eval)), rep("Recent", length(df_evals$recent_eval)))
...
ggboxplot(data_df_evals_transf, x = "Evaluation", y = "Score", 
  ...
  ggtitle("Boxplot for first and recent evaluations in all Health Zones")



paste("Wilcoxon Test outputs:")

wilcox.test(first_eval, recent_eval, paired=TRUE)
...
data_nziza_but_kat <- dplyr::filter(first_last_df, first_last_df$zone_de_sante=="butembo" | first_last_df$zone_de_sante=="katwa")

first_eval <- data_nziza_but_kat$`1st_score_pci_100`
recent_eval <- data_nziza_but_kat$last_score_pci_100
...
ggboxplot(data_df_evals_transf, x = "Evaluation", y = "Score", 
          color = "Evaluation", palette = c("#FF0000", "#00FF00"),
          ylab = "IPC Score", xlab = "IPC evaluations")+
  ggtitle("Boxplot for first and recent evaluations in Butembo and Katwa")
...
## Descriptive analysis of first evaluations by Health Zone
...
dat_1 <- dat_1 %>% dplyr::rename(score_pci_first = `1st_score_pci_100` )

stats_overall <- ggpubr::desc_statby(dat_1, measure.var = "score_pci_first",
                                     grps = "zone_de_sante")
...
x_d = summary(dat_1$score_pci_first)
...
stats_overall_c %>%
  ...
  kableExtra::add_header_above( c( "Summary statistics for IPC score for first evaluations by Health Zones" = ncol(stats_overall_c)) ) %>%
...
plot.stats_overall <- ggplot(stats_overall, aes(x=zone_de_sante, y=median, group = zone_de_sante)) + 
  ...
  ggtitle("Summary statistics for IPC score for first evaluations by Health Zones")
...
dat_2 <- dat_2 %>% rename(score_pci_recent = last_score_pci_100)
